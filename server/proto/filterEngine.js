...
const CLEAN_RE = new RegExp(
  `^ask\\s+(?:those\\s+(?:who\\s+)?|if\\s+)?(?:not\\s+coded|coded)\\s+[\\d,\\s/&-]+\\s+in\\s+${QID}` +
  `(?:\\s+(?:and|or)\\s+(?:not\\s+coded|coded)\\s+[\\d,\\s/&-]+\\s+in\\s+${QID})*\\s*$`, 'i');
...
function parseOneClause(s) {
  ...
  return { neg, q: q.toUpperCase(), codes };
}
...
function renderClause(c, first) {
  ...
  const body = `${c.neg ? 'not coded' : 'coded'} ${codeStr} in ${c.q}`;
